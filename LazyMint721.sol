...
// Meeflex Contracts (last updated v1.0.0) (lib/Marketplace.sol)
...
/**
 * @title Meeflex Marketplace library.
 */
library LazyMint721 {
    struct TokenData {
        uint id;
    ...
    function hashMint(address collectionAddress, TokenData memory _tokenData, MintData memory _saleData) internal pure returns (bytes32) {
    ...
    struct SaleDataSeller {
        address bidder;
        uint price;
        uint biddingExpTimestamp;
        uint nodeExpTimestamp;
    }

    bytes32 public constant LISTING_TYPEHASH_SELLER = keccak256("ListingSeller(address collectionAddress,uint id,uint price,uint expTimestamp)");
    function hashListingSeller(address collectionAddress, TokenData memory _tokenData, SaleDataSeller memory _saleData, bool isNode) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
                LISTING_TYPEHASH,
                collectionAddress,
                _tokenData.id,
                _saleData.price,
                isNode ? _saleData.nodeExpTimestamp : _saleData.biddingExpTimestamp
            )
        );
    }
}
